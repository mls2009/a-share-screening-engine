...
def ma_pierce_2y_scan(frame: pd.DataFrame) -> pd.Series:
    ...
    averages = frame[[f"ma_{window}" for window in MA_PIERCE_WINDOWS]].astype(float)
    pierced = (
        (closing > opening)
    ...
    slopes = pd.DataFrame(
        {
            window: frame[f"ma_{window}"].astype(float).rolling(5).apply(
    ...
    entangled = spread <= ENTANGLE_SPREAD_MAX_PERCENT
    voluminous = volume >= VOLUME_MULTIPLE * frame["volume_ma_20"].astype(float)
    required = ["open", "close", "volume", "volume_ma_20", *(f"ma_{w}" for w in MA_PIERCE_WINDOWS)]
    complete = frame[required].notna().all(axis=1)
    return (pierced & flat & entangled & voluminous & complete).fillna(False)


def ma_pierce_2y_hits(frame: pd.DataFrame) -> list[dict]:
    """扫描窗口内所有满足完整形态的日期（升序）。"""
...
def ma_pierce_2y_exists(frame: pd.DataFrame) -> pd.Series:
    """逐行：截至该行（含）的过去约两个自然年内是否出现过完整形态。

    供回测逐 bar 评估使用；窗口长度 TWO_YEARS，dates 需升序。
    """
    pattern = ma_pierce_2y_scan(frame).to_numpy(dtype=bool)
