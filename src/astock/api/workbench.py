...
from astock.features.ma250_breakout import uses_ma250_breakout
...
def register_workbench(app, context, add_watchlist, watch_request):
    ...
    def waves(symbol: str, payload: WaveRequest):
        ...
        rows = MarketFeatureStore(context.database).read_history(symbol, Timeframe.DAY, datetime.now(SHANGHAI).date(), count, enrich=False, include_vacuum=uses_vacuum(payload.tree), include_shapes=uses_shapes(payload.tree), include_price_action=uses_price_action(payload.tree), include_ma_support=uses_ma_support(payload.tree), include_ma250_reclaim=uses_ma250_reclaim(payload.tree), include_ma250_breakout=uses_ma250_breakout(payload.tree), include_ma_pierce=uses_ma_pierce(payload.tree))
    ...
    def detail(run_id: UUID, symbol: str, latest: bool = False, days: int = Query(20, ge=1, le=60)):
        ...
        histories = {timeframe: store.read_history(symbol, timeframe, as_of, 1100, enrich=timeframe.value in enriched_frames, include_vacuum=uses_vacuum(record["tree"]), include_shapes=uses_shapes(record["tree"]), include_price_action=uses_price_action(record["tree"]), include_ma_support=uses_ma_support(record["tree"]), include_ma250_reclaim=uses_ma250_reclaim(record["tree"]), include_ma250_breakout=uses_ma250_breakout(record["tree"]), include_ma_pierce=uses_ma_pierce(record["tree"])) for timeframe in Timeframe}
