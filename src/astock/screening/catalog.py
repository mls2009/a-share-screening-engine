...
METRICS = [
    ...
    *[MetricSpec(key, label, Unit.BOOLEAN, timeframes=frozenset({Timeframe.WEEK if key.startswith("pw_") else Timeframe.DAY}),
    ...
    MetricSpec("ma_cluster_pierce_up_2y", "近两年：MA10/20/30走平粘连时，放量大阳实体上穿除年线外全部均线", Unit.BOOLEAN,
               timeframes=frozenset({Timeframe.DAY}), operators=EQUALITY_OPERATORS, group="trend",
               family="ma_cluster_pierce",
               choices=(ChoiceSpec("true", "是"), ChoiceSpec("false", "否")),
               supported_modes=frozenset({"close", "backtest"})),
    MetricSpec("ma250_volume_breakout_2y", "近两年：年线放量突破（2倍量且振幅>5%）或涨停突破", Unit.BOOLEAN,
               timeframes=frozenset({Timeframe.DAY}), operators=EQUALITY_OPERATORS, group="trend",
               family="ma250_volume_breakout", choices=(ChoiceSpec("true", "是"), ChoiceSpec("false", "否")),
               supported_modes=frozenset({"close"})),
    MetricSpec("ma250_reclaim_2d_2y", "近两年：收盘跌破年线后1～2日内收复", Unit.BOOLEAN,
               timeframes=frozenset({Timeframe.DAY}), operators=EQUALITY_OPERATORS, group="trend",
    ...
    MetricSpec("ma120_250_repeat_support_2y", "近5交易日：MA120/250两次有效支撑后再次出现下影（含双K）", Unit.BOOLEAN,
               ...
               supported_modes=frozenset({"close"})),
    MetricSpec("ma_cluster_pierce_10d_no120_2y", "近两年：10日放平粘连0.8%·放量穿线（不含MA120）", Unit.BOOLEAN,
               timeframes=frozenset({Timeframe.DAY}), operators=EQUALITY_OPERATORS, group="trend",
               family="ma_cluster_pierce",
               choices=(ChoiceSpec("true", "是"), ChoiceSpec("false", "否")),
               supported_modes=frozenset({"close", "backtest"})),
