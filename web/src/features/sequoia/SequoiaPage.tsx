...
export function SequoiaPage() {
  ...
  const visible = (run?.matches ?? []).filter(m => !filter || m.groups.some(g => g.id === filter)).sort((a, b) => sort === 'change' ? (b.change_percent ?? -Infinity) - (a.change_percent ?? -Infinity) : a.symbol.localeCompare(b.symbol));
  ...
  function view(next: Run) { setRun(next); setSelected([]); setDetail(undefined); setFocus(undefined); setHiddenStrategies([]); setFilter(''); setNotice(''); }
