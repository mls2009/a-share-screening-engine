...
export function metricHelp(metric: MetricSpec): string {
  if (metric.key === "ma250_volume_breakout_2y") return "扫描最近两个自然年全部日线命中：前一交易日收盘≤其当天MA250，当日收盘>当日MA250。突破当日满足任一分支：①成交量≥突破前20个交易日平均成交量的2倍（不含当天，须有完整20日数据），且（最高价－最低价）÷前收盘价×100严格>5%；②真实收盘涨停，豁免成交量和振幅要求，一字涨停也可。涨停按本地未复权收盘价与当日涨停价核验，缺少数据不能使用涨停豁免。两年内所有符合日期都在K线上标记，不限制最近5日。";
  if (metric.key === "ma250_reclaim_2d_2y") return "扫描近两个自然年内的日线跌破事件：前一根收盘价>其当天MA250，跌破当日收盘价<其当天MA250；随后第1或第2个交易日收盘价>其当天MA250即命中。只记录首次收复；等于均线不算站上或跌破。不限制成交量或影线，保留全部历史命中并在图上标出跌破日至收复日。";
  if (metric.key === "ma250_reclaim_3d_shadow_recent5_2y") return "以下任一满足即可，跌破日至收复日、或双K的首日至末日，都必须完整落在市场最近5个已入库交易日内：①前一根收盘>当天MA250，当根收盘<当天MA250，随后第1～3根收盘重新>各自当天MA250，仅记录首次收复。②单K或双K合并后形成下影支撑：最低价距其当天MA250在±2%以内，下影>0且≥实体，最终收盘>当天MA250。双K使用首根开盘、末根收盘和两根最低价合成。不要求此前两次有效支撑；不限制成交量。扫描近两年背景，图上标记最近5日命中的实际区间和形态类别。";
