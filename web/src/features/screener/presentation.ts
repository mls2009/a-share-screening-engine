...
export function metricHelp(metric: MetricSpec): string {
  if (metric.key === "ma120_250_repeat_support_2y") return "两年窗口开头20个交易日不计支撑。从第21日起，MA120按股价分位取值后要求当日K线最低价≤均线≤最高价，MA250仍允许最低价距离均线±2%。同一均线连续两次触及后，各有20日收盘高于对应均线，确认完成才开始下一次；期间及之后任一天收盘≤均线即清零。两次确认完成后才记录第三次及后续单K/双K下影支撑：下影>0且≥实体，最终收盘在均线上方。";
