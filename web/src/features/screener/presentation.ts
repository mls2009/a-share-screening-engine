...
export function metricHelp(metric: MetricSpec): string {
  if (metric.key === "ma250_reclaim_2d_2y") return "扫描近两个自然年内的日线跌破事件：前一根收盘价>其当天MA250，跌破当日收盘价<其当天MA250；随后第1或第2个交易日收盘价>其当天MA250即命中。只记录首次收复，第1日已收复则不重复记录第2日。等于均线不算站上或跌破；无需实体全部越过，也不限制成交量或影线。保留全部历史命中，在图上框出跌破日至收复日，不要求最近5日发生。";
