...
export function metricHelp(metric: MetricSpec): string {
  if (metric.key === "ma250_reclaim_2d_2y") return "扫描近两个自然年内的日线跌破事件：前一根收盘价>其当天MA250，跌破当日收盘价<其当天MA250；随后第1、第2或第3个交易日收盘价>其当天MA250即确认收复；仅收复日发生在市场最近5个已入库交易日内才入选。只记录首次收复，后续站在线上不重复计数。等于均线不算站上或跌破；无需实体全部越过，也不限制成交量或影线。保留最近5个交易日的全部命中，在图上框出跌破日至收复日。";
