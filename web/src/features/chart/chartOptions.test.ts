...
const zone = (
  ...
  slope: null, intercept: null, anchors: [], strength: .8, touches: 3, source, reappeared,
});

it("支撑为绿色、压力为红色，只有删除后重现的线使用虚线", () => {
  expect(zonePresentation(zone("support", "auto"))).toMatchObject({ color: "#2ecf79", lineType: "solid", label: "自动支撑" });
