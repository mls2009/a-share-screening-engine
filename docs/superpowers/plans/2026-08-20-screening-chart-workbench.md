...
- [ ] 先写失败测试：特征 JSON 展开、形态/支撑压力并入筛选快照、手动区间 CRUD。
- [ ] 实现并验证：手动数据不被自动重算删除，自动与手动区间可同时返回。
- [ ] 扩展条件目录，覆盖新高/新低、回撤、连涨/连跌、形态与区间距离。
...
- [ ] 先写失败测试：页签、数据状态、选股页首屏。
- [ ] 实现深色交易终端外壳，支持宽屏和窄屏，数字使用等宽字体。
- [ ] Vite 开发代理 `/api` 到 FastAPI，生产构建输出由 FastAPI 托管。
...
- [ ] 先写失败测试：新增/删除条件、嵌套 AND/OR/NOT、常量/指标对比、校验错误。
- [ ] 根据后端 catalog 动态渲染指标、周期、操作符和单位，不在前端重复业务规则。
- [ ] 运行收盘/实时筛选，展示排名、证券名称、关键数值和逐层命中解释。

### Task 4: 实现 K 线、成交量与支撑压力绘图

**Files:** `web/src/features/chart/StockChart.tsx`, `DrawingToolbar.tsx`, `ChartPage.tsx`, 对应 `*.test.tsx`

- [ ] 先写失败测试：5/15/30/60 分钟、日/周/月切换；颜色和线型映射；手动线提交。
- [ ] K 线与成交量共享时间轴、缩放和十字光标。
- [ ] 支撑使用绿色，压力使用红色；自动区间半透明虚线，手动区间/趋势线实线并带“手动”标识。
- [ ] 手动模式先选支撑或压力，再用两个锚点创建水平区间或趋势区间，可删除。

### Task 5: 端到端验收与本地启动

**Files:** `src/astock/api/app.py`, `tests/integration/test_web_workbench.py`, `README.md`

- [ ] 构建前端，从 FastAPI 首页打开工作台。
- [ ] 以固定数据验收“过去 20 个交易周期涨幅≥30%”、查看 K 线、自动区间和保存手动趋势线。
- [ ] 运行 `uv run --extra dev pytest -q`、`uv run --extra dev ruff check src tests`、`npm test -- --run`、`npm run build`。
