...
- `GET /api/symbols/{symbol}/zones`：返回最近的自动水平支撑/压力、所有处于 `active`
  状态的手动线，以及每个方向最多 1 条自动上升/下降趋势线。
...
图表只绘制区域中心线：绿色显示支撑，红色显示压力。自动线在图上不常驻显示
名称或数值，直接悬停到线上才显示；手动画线则始终保留用户选择的支撑/压力角色。
自动线和手动画线都能删除；普通结果统一使用实线，被删除点位以后落入原价格容差并
再次生成时使用虚线。水平线和趋势线都支持；自动水平线会按现价归类为支撑或压力，自动
趋势线则按斜率归类为上升或下降趋势。默认选取离现价最近的 3 条自动水平支撑和 3 条
自动水平压力，并在有效条件满足时每个方向最多选取 1 条自动趋势线。
